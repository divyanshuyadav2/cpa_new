...
<div class="mb-8">
    ...
    <h1 class="text-3xl font-extrabold text-pharma-navy">Add New Salt / Composition</h1>
</div>

<div class="max-w-2xl bg-white border border-slate-200 rounded-2xl shadow-sm p-6 sm:p-8">
    <form action="{{ route('salts.store') }}" method="POST" class="space-y-6">
        @csrf

        <!-- Salt Name -->
        <div>
            <label for="name" class="block text-xs font-bold uppercase tracking-wider text-slate-700 mb-2">Salt Name</label>
            <input type="text" name="name" id="name" required value="{{ old('name') }}" placeholder="e.g. Paracetamol" 
        ...
        <div>
            ...
            <textarea name="description" id="description" rows="4" placeholder="Brief details about the composition and typical applications..." 
