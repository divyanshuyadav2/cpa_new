...
<div class="mb-8">
    ...
    <h1 class="text-3xl font-extrabold text-pharma-navy">Add New Product</h1>
</div>
    ...
    <form action="{{ route('products.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <input type="text" name="name" id="name" required value="{{ old('name') }}" placeholder="e.g. Paracip 650" 
            ...
            <div>
                ...
                <select name="salt_id" id="salt_id" required
                    ...
                    @foreach($salts as $salt)
                        <option value="{{ $salt->id }}" {{ old('salt_id') == $salt->id ? 'selected' : '' }}>
                            {{ $salt->name }}
                        </option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <input type="text" name="composition" id="composition" required value="{{ old('composition') }}" placeholder="e.g. Paracetamol IP 650mg" 
            ...
            <div>
                ...
                <input type="number" step="0.01" name="mrp" id="mrp" required value="{{ old('mrp') }}" placeholder="0.00" 
            ...
            <div>
                ...
                <input type="number" step="0.01" name="ptr" id="ptr" required value="{{ old('ptr') }}" placeholder="0.00" 
            ...
            <div>
                ...
                <input type="number" name="stock_qty" id="stock_qty" required value="{{ old('stock_qty', 0) }}" placeholder="0" 
            ...
            <!-- Product Image -->
            <div>
                <label for="image" class="block text-xs font-bold uppercase tracking-wider text-slate-700 mb-2">Product Formulation Image</label>
                ...
                <span class="text-[10px] text-slate-400 mt-1 block">Upload formulation image (Max 2MB).</span>
                @error('image')
                    <p class="text-xs text-red-500 mt-1 font-semibold">{{ $message }}</p>
                @enderror
            </div>
            ...
            fetchDivisions() {
                ...
                fetch(`/admin/divisions/by-company?company_id=${this.selectedCompany}`)
                    ...
                    .then(data => {
                        ...
                        // Wait for Alpine to render template options before binding division
                        this.$nextTick(() => {
