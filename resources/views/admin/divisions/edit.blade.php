...
<div class="mb-8">
    ...
    <h1 class="text-3xl font-extrabold text-pharma-navy">Edit Division Details</h1>
</div>
...
<div class="max-w-2xl bg-white border border-slate-200 rounded-2xl shadow-sm p-6 sm:p-8">
    <form action="{{ route('divisions.update', $division->id) }}" method="POST" class="space-y-6">
        ...
        <div>
            ...
            <input type="text" name="name" id="name" required value="{{ old('name', $division->name) }}" placeholder="e.g. Cipla Diagnostics" 
        ...
        <div>
            ...
            <textarea name="description" id="description" rows="4" placeholder="Brief details about the division..." 
                      class="w-full px-3.5 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-pharma-accent focus:bg-white @error('description') border-red-500 @enderror">{{ old('description', $division->description) }}</textarea>
            @error('description')
                <p class="text-xs text-red-500 mt-1 font-semibold">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div class="flex items-center">
            <input type="checkbox" name="is_active" id="is_active" value="1" {{ old('is_active', $division->is_active) ? 'checked' : '' }}
